...
-- ENUM TYPES
-- =========================
DO $$ BEGIN CREATE TYPE user_role AS ENUM ('customer', 'vendor', 'rider');
...
DO $$ BEGIN CREATE TYPE payment_method AS ENUM ('cash', 'gcash');
...
-- =========================
-- NOTE: Using "id" directly (no renaming, no schema mutation)
...
CREATE TABLE IF NOT EXISTS store_item (
  ...
  image_url TEXT,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_store_item_store FOREIGN KEY (store_id) REFERENCES store(store_id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS "order" (
  ...
  CONSTRAINT fk_order_customer FOREIGN KEY (customer_id) REFERENCES "user"(id) ON DELETE CASCADE,
  CONSTRAINT fk_order_store FOREIGN KEY (store_id) REFERENCES store(store_id) ON DELETE CASCADE,
  CONSTRAINT fk_order_rider FOREIGN KEY (rider_id) REFERENCES "user"(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS order_item (
  ...
  CONSTRAINT fk_order_item_store_item FOREIGN KEY (store_item_id) REFERENCES store_item(store_item_id) ON DELETE RESTRICT
);
...
DROP INDEX IF EXISTS idx_order_rider_id;
DROP INDEX IF EXISTS idx_order_store_id;
DROP INDEX IF EXISTS idx_order_customer_id;
DROP INDEX IF EXISTS idx_store_item_store_id;
...
-- =========================
-- USER COLUMN CHANGES
-- NOTE: No changes applied to auth schema beyond optional columns
-- =========================
